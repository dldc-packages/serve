...
const ALL_STATUS_BY_CODE: { [K in HttpStatusCode]: HttpStatusObject } = Object
  .fromEntries(
    Object.entries(ALL_STATUS).map((
      [code, infos],
    ) => [code, { code: parseInt(code), ...infos }]),
  ) as any;

const ALL_STATUS_BY_NAME: { [K in HttpStatusName]: HttpStatusObject } = Object
  .fromEntries(
    Object.entries(ALL_STATUS).map((
      [code, infos],
