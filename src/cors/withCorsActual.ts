...
export function withCorsActual(res: ZenResponse, cors: CorsConfigResolved): ZenResponse {
  return res.withHeaders((prev) => {
    ...
    if (cors.exposeHeaders && cors.exposeHeaders.length > 0) {
      nextHeaders.set(HttpHeader.AccessControlExposeHeaders, cors.exposeHeaders.join(', '));
    }
    return nextHeaders;
  });
}
