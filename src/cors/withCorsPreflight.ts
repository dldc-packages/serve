...
export function withCorsPreflight(config: CorsPreflightConfigResolved): ZenResponse {
  ...
  if (config.allowMethods && config.allowMethods.length > 0) {
    headers.set(HttpHeader.AccessControlAllowMethods, config.allowMethods.join(', '));
  }
  if (config.allowHeaders && config.allowHeaders.length > 0) {
    headers.set(HttpHeader.AccessControlAllowHeaders, config.allowHeaders.join(', '));
  }
  if (config.exposeHeaders && config.exposeHeaders.length > 0) {
    headers.set(HttpHeader.AccessControlExposeHeaders, config.exposeHeaders.join(', '));
  }
